...
# ----------------- Setup -----------------
os.chdir(r"C:\Users\abdel\Instagram_Project")
...
def transform_data(df):
   ...
   # ---- Outlier Detection (IQR method) on engagement_rate ----
    Q1 = df["engagement_rate"].quantile(0.25)
    Q3 = df["engagement_rate"].quantile(0.75)
    IQR = Q3 - Q1
    lower_bound = Q1 - 1.5 * IQR
    upper_bound = Q3 + 1.5 * IQR
    df["is_outlier"] = (~df["engagement_rate"].between(lower_bound, upper_bound)).astype(int)
    print(f"Flagged {df['is_outlier'].sum()} outlier posts out of {len(df)} "
    ...
    instagram_fact = instagram_fact[[
        ...
        "high_engagement_flag", "avg_engagement_by_media", "is_outlier",
        "traffic_source"
    ]]
...
def analyze_data(df):
    ...
    print("Correlation with engagement_rate (top 5 by |r|):")
    print(corr.head())
 
    # ---- Outlier-Adjusted Category Comparison ----
    ...
    category_check = df.groupby("content_category").apply(
        lambda g: pd.Series({
            ...
            "mean_er_no_outliers": g.loc[g["is_outlier"] == 0, "engagement_rate"].mean(),
            "n_outliers": g["is_outlier"].sum()
        })
    ).reset_index()
    category_check.to_csv(f"{processed_dir}/category_outlier_check.csv", index=False)
    print("\nCategory engagement rate — all posts vs. outliers removed:")
    print(category_check)
 
    return corr, category_check
